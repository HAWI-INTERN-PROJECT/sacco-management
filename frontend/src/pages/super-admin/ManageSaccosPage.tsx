...
import { ChevronLeft, ChevronRight, Loader2, RefreshCw, Search, Download, Building2, Check, X } from 'lucide-react'
  ...
  useEffect(() => {
    const fetchCounts = async () => {
      try {
        ...
        // Silently ignore tab count errors
      }
    }
    fetchCounts()
  }, [])
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-3 shrink-0 flex-wrap">
          ...
          <button
            ...
            disabled={exporting}
            className="bg-[#0B1727] hover:bg-[#0B1727]/90 text-white px-4 py-2 rounded-lg text-xs font-bold shadow-2xs transition-colors disabled:opacity-50 inline-flex items-center gap-2"
          >
                      ...
                      {sacco.status === 'pending' ? (
                        <div className="flex items-center justify-center gap-2">
                          <button
                            ...
                            className="px-3 py-1 bg-[#10B981] hover:bg-emerald-600 text-white text-xs font-bold rounded-lg transition-colors shadow-2xs disabled:opacity-50 inline-flex items-center gap-1"
                          >
                          ...
                          <button
                            ...
                            className="px-3 py-1 bg-white border border-rose-200 hover:bg-rose-50 text-rose-600 text-xs font-bold rounded-lg transition-colors disabled:opacity-50 inline-flex items-center gap-1"
                          >
                            <X className="w-3 h-3" />
                            <span>Reject</span>
                          </button>
        ...
        <div className="px-6 py-4 border-t border-slate-100 flex flex-col sm:flex-row items-center justify-between gap-4 text-xs text-slate-500">
          ...
          <div className="flex items-center gap-1.5 font-semibold">
            <button
              ...
              disabled={Boolean(!meta || meta.current_page <= 1 || loading)}
            ...
            <button
              ...
              disabled={Boolean(!meta || meta.current_page >= meta.last_page || loading)}
              className="p-1 rounded text-slate-600 hover:text-slate-900 disabled:text-slate-300 disabled:cursor-not-allowed transition-colors"
            >
